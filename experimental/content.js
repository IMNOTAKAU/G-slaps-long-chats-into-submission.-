(function () {
  ...
  function doCollapse(el) {
    if (collapsed.has(el) || !el.parentNode) return;
  ...
  function doRestore(el) {
    ...
    selfMutating = true;
    if (data.spacer.parentNode) data.spacer.parentNode.replaceChild(el, data.spacer);
    selfMutating = false;
    collapsed.delete(el);
  ...
  function restoreAll() {
    ...
    for (const [el, data] of collapsed) {
      if (data.spacer && data.spacer.parentNode) data.spacer.parentNode.replaceChild(el, data.spacer);
    }
    selfMutating = false;
    collapsed.clear();
